...
Etape 13 - requalification docker de l'image 1.1.0 (session docker active requise : le groupe docker ne s'applique qu'a l'ouverture d'une nouvelle session). Sequence pilotee par l'agent :

1. **Prealable fait** (commit pre-etape 13) : regles kit KIT-TEST passees en `any any -> any any` (bug `$HOME_NET` corrige : les IP synthetiques RFC 5737 de l'E2E ne sont pas dans le HOME_NET Debian, le recall 5ter echouerait) ; `suricata-update` ajoute a l'inventaire importlib de gen_licences.sh
2. `docker version` - si acces refuse : fournir le bloc de commandes exact en guidance (build, test, E2E, licences) et analyser les retours
3. `python3 scripts/doctor.py check` (barriere 3 Go build) puis `python3 scripts/doctor.py fix` - build `oreoa-ai-tools:1.1.0` (ET Open snapshot + triage + regles kit + trace bakee) - **consigner le digest de l'image** (tracabilite forensique)
4. `python3 scripts/doctor.py test` - 8 outils + bibliotheques + copyright (suricata inclus) + E2E ; verifier in-image : `/etc/suricata/kit/regles-trace.txt` (SHA256 du ruleset) et la reference `threshold-file` dans la config Debian (si absente : ajouter `--set threshold-file=...` a la fiche connaissances et a l'invocation E2E)
5. E2E conteneurise : etape 5 (yara, log2timeline), 5bis (skip attendu, pas de dump), **5ter** (tshark DNS C2 ; suricata recall sur c2.pcap - alertes KIT-TEST sids 1000001/1000002 attendues ; bruit sur clean.pcap - zero alerte attendu, alertes ET Open residuelles en warn)
6. Boucle de tri autonome jusqu'au zero alerte sur clean.pcap : resserrer `config/suricata/disable.conf` ou `threshold.config`, rebuild (cache : seules COPY/rules/LABEL se reconstruisent), re-test. **Politique de journalisation obligatoire** : toute regle ou categorie desactivee est consignee deux fois - commentaire justifie dans le fichier `config/suricata/` (regle + raison + effet constate) et entree dans le journal ci-dessous
7. `scripts/gen_licences.sh` -> regen `docs/licences-image.txt` (inventaire elargi avec suricata) ; NOTICE re-verifiee face aux textes copyright reels (suricata GPL-2.0, suricata-update GPLv2)
8. Cloture : MEMORY.md (etape 13 terminee, journal, prochaine action -> roadmap v2), fiche projet, _System/MEMORY.md, commit + push

Modes d'echec anticipes au build : pin `suricata=7.0.10-1+deb13u4` depasse (si deb13u5 publie, relever la version du pool Debian et maj Dockerfile) ; flags `suricata-update --no-test/--disable-file` differents en 1.3.3 (corriger le Dockerfile) ; suricata-update exigeant un update.yaml (config minimale explicite). Rebuild apres correction : cache apt/pip preserve.
